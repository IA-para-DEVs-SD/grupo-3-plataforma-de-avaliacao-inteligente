...
describe("LoginForm", () => {
  ...
  it("deve desabilitar campos durante o envio", async () => {
    ...
    // Campos devem estar desabilitados durante o carregamento
  ...
  it("deve limpar erro da API ao tentar submeter novamente", async () => {
    mockLogin.mockRejectedValueOnce({
      response: { data: { error: { message: "Credenciais inválidas" } } },
    });
    mockLogin.mockResolvedValueOnce({ token: "abc", user: { id: "1" } });
    ...
    // Segunda tentativa — erro deve sumir
    await user.click(screen.getByRole("button", { name: "Entrar" }));

    await waitFor(() => {
      expect(
        screen.queryByText("E-mail ou senha incorretos"),
      ).not.toBeInTheDocument();
    });
  });

  it("deve marcar aria-invalid como true quando campo tem erro", async () => {
    const user = userEvent.setup();
    renderForm();
