...
describe("ReviewForm — formulário de avaliação", () => {
  ...
  describe("usuário autenticado", () => {
    ...
    it("deve exibir erro de validação quando nota não é selecionada", async () => {
      ...
      await user.type(
        screen.getByLabelText("Sua avaliação"),
        "Este é um texto com mais de vinte caracteres para teste",
      );
      await user.click(screen.getByText("Enviar avaliação"));
    ...
    it("deve chamar onSubmit com dados válidos e limpar formulário", async () => {
      ...
      await user.type(
        screen.getByLabelText("Sua avaliação"),
        "Produto muito bom, recomendo para todos!",
      );
      await user.click(screen.getByText("Enviar avaliação"));
    ...
    it("deve exibir estado de carregamento no botão durante envio", async () => {
      const user = userEvent.setup();
      let resolveSubmit;
    ...
    it("deve exibir ambos os erros quando texto e nota são inválidos", async () => {
      const user = userEvent.setup();
      renderWithRouter(<ReviewForm onSubmit={vi.fn()} />);
      ...
      expect(screen.getByText("A nota é obrigatória")).toBeInTheDocument();
      expect(
        screen.getByText("O texto da avaliação é obrigatório"),
      ).toBeInTheDocument();
    });

    it("deve ter aria-label acessível no formulário", () => {
      renderWithRouter(<ReviewForm onSubmit={vi.fn()} />);
