...
describe('analyzeSentiment', () => {
  it('deve retornar "positive" para texto com mais palavras positivas', () => {
    expect(analyzeSentiment('Produto excelente, ótimo e de qualidade!')).toBe('positive');
  });

  it('deve retornar "negative" para texto com mais palavras negativas', () => {
    expect(analyzeSentiment('Produto péssimo, horrível, um lixo total')).toBe('negative');
  });
  ...
  it('deve retornar "neutral" para texto sem palavras-chave', () => {
    expect(analyzeSentiment('Comprei este produto na loja do centro')).toBe('neutral');
  });
