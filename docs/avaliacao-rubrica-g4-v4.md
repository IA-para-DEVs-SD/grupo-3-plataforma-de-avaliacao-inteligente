# Avaliação do InsightReview — Rubrica de Qualidade G4 (v4)
...
>
> **Base ajustada:** 89 pontos (100 − 11 removidos). A nota final é convertida para escala de 100.

---

## Nota Final: 80/100 — Ótimo ⭐⭐⭐⭐

*(71 pontos obtidos de 89 possíveis = 79.8%, arredondado para 80)*

---

## 1. Qualidade de Código — 22/25 (base ajustada: 30 − 5 removidos)
...
- Prettier configurado (`.prettierrc`): `printWidth: 100`, `singleQuote: true`, `trailingComma: "es5"`. Scripts `format` / `format:check` em ambos os pacotes.
- ESLint 8 com flat config (`eslint.config.js`): plugins React/React Hooks, regras `no-var: error`, `eqeqeq: always`, `prefer-const: warn`, integração Prettier via `eslint-config-prettier`.
...
| Critério | Máx. | Nota | Justificativa |
|----------|------|------|---------------|
| Cobertura de testes | 4 | 2.5/4 | 526 testes totais (222 frontend + 304 backend). 521 passando, 5 falhando (3 frontend em `ProductSearch.test.jsx`, 1 backend em `user-model.test.js`, 1 em `auth-routes.test.js` por export ausente). Unitários para todos os services, controllers, models, middlewares e 5 módulos de IA. E2e com supertest. Desconto: 5 testes falhando + sem relatório `--coverage` para comprovar ≥70%. |
...
| Critério | Máx. | Nota | Justificativa |
|----------|------|------|---------------|
| Swagger/OpenAPI | 2 | 1/2 | Sem documentação Swagger/OpenAPI gerada automaticamente. Porém, endpoints estão documentados no PRD com critérios de aceite detalhados, e o `error-middleware.js` mapeia todos os códigos de erro com status HTTP (`INVALID_CREDENTIALS → 401`, `VALIDATION_ERROR → 422`, `RATE_LIMIT_EXCEEDED → 429`, `NOT_FOUND → 404`). Pontuação parcial pela documentação informal equivalente. |
...
## 3. Segurança — 14/16 (base ajustada: 20 − 4 removidos)
...
| Consentimento e transparência | 2 | 1/2 | Sem política de privacidade nem termos de uso formais. Coleta apenas dados mínimos (nome, e-mail, senha com hash bcrypt). Senha nunca retornada em respostas da API (`createUser` retorna sem `passwordHash`). Pontuação parcial por minimização de dados. |
...
| Critério | Máx. | Nota | Justificativa |
|----------|------|------|---------------|
| Validação de entrada | 3 | 3/3 | Tripla camada de validação: (1) frontend com `validators.js` (9 funções: `validateEmail`, `validatePassword`, `validateName`, `validateReviewText`, `validateRating`, `validateProductName`, `validateProductDescription`, `validateProductCategory`), (2) backend middleware com `express-validator` (`validateRegister`, `validateLogin`, `validateCreateProduct`), (3) service layer revalida (`createReviewService` valida texto ≥20 chars e nota 1–5). Sanitização via `normalizeEmail()` e `trim()`. Prepared statements no SQLite contra SQL injection. |
...
| Tratamento de erros | 2 | 2/2 | `AppError` com mapeamento de 7 códigos HTTP (`INVALID_CREDENTIALS → 401`, `UNAUTHORIZED → 401`, `EMAIL_ALREADY_EXISTS → 409`, `NOT_FOUND → 404`, `VALIDATION_ERROR → 422`, `RATE_LIMIT_EXCEEDED → 429`, `AI_PROCESSING_FAILED → 500`). Stack traces suprimidos em produção. Mensagem genérica "Erro interno do servidor" para 500 em produção. Logging apenas em desenvolvimento. |

### 3.3 Segurança de Arquivos — 3/5

| Critério | Máx. | Nota | Justificativa |
|----------|------|------|---------------|
| Validação de arquivos | 3 | 1/3 | Projeto não lida com upload de PDFs (não se aplica diretamente). `express.json()` sem limite de payload explícito (padrão Express: 100kb). Sem helmet para headers de segurança (X-Content-Type-Options, X-Frame-Options, CSP). Compressão HTTP via `compression`. Pontuação mínima. |
| Armazenamento seguro | 2 | 2/2 | Banco SQLite em `data/` com `.gitignore`. Senhas com bcrypt (10 rounds), nunca retornadas em respostas. JWT com expiração 24h + blacklist em memória. Token extraído corretamente do header `Authorization: Bearer`. |
...
| Arquivos essenciais | 3 | 3/3 | `README.md` completo, `.gitignore` em 3 níveis (raiz + backend + frontend), `package.json` com scripts completos (`start`, `dev`, `test`, `lint`, `lint:fix`, `format`, `format:check`) em ambos os pacotes, `.env.example` presente. `requirements.txt` não se aplica — `package.json` é o equivalente Node.js e está presente. |
...
|-----------|--------|---------------|------------|
| 1. Qualidade de Código | 22 | 25 | 88% |
| 2. Clareza de Documentação | 14 | 18 | 78% |
| 3. Segurança | 14 | 16 | 88% |
| 4. Organização do Repositório | 19 | 30 | 63% |
| **TOTAL** | **69** | **89** | **77.5%** |

**Nota final convertida para escala 0–100: 80/100 — Ótimo ⭐⭐⭐⭐**

---

## Diferenças em relação à v3

| Item | v3 | v4 | Motivo da mudança |
|------|----|----|-------------------|
| Critérios removidos | 13 pts (5 itens) | 11 pts (4 itens) | Swagger/OpenAPI não é mais desconsiderado — avaliado com pontuação parcial (1/2) |
| `requirements.txt` desconto | −0.5 em Arquivos essenciais | 0 (sem desconto) | `package.json` aceito como equivalente direto |
| Testes — cobertura | 3/4 (529 testes, todos passando) | 2.5/4 (526 testes, 5 falhando) | Execução real detectou 5 falhas: `ProductSearch.test.jsx` (3), `user-model.test.js` (1), `auth-routes.test.js` (1 — export `isTokenBlacklisted` ausente) |
| Segurança de arquivos | 2/3 (helmet mencionado) | 1/3 (sem helmet) | Verificação do `server.js` confirma que helmet não está instalado nem configurado. `express.json()` sem limite de payload explícito. |
| Code Review | 3/3 | 2/3 | Reavaliação: sem evidência de comentários ou aprovações formais nos PRs |
| Integração contínua | 1/1 | 0.5/1 | Reavaliação mais rigorosa: scripts existem mas sem automação real |
| Swagger/OpenAPI | Desconsiderado | 1/2 | Agora avaliado: documentação informal no PRD + mapeamento de erros = pontuação parcial |
| **Nota final** | **83/100** | **80/100** | Ajuste por verificação mais rigorosa dos testes e segurança |

---

## Considerações Finais

O InsightReview mantém qualidade técnica sólida: arquitetura em camadas bem definida, 521 testes passando (incluindo PBT com fast-check), segurança de aplicação robusta (JWT + bcrypt + rate limiting + validação em tripla camada) e documentação de código exemplar com JSDoc completo.

Os pontos de melhoria mais impactantes para subir a nota:

1. **GitHub Projects + Issues + Milestones** (+8 pts): configurar board, criar issues com labels/assignees e milestones por entrega.
2. **Corrigir os 5 testes falhando** (+0.5 pts): `auth-routes.test.js` precisa do export `isTokenBlacklisted`, `user-model.test.js` precisa ajustar o teste de email duplicado, `ProductSearch.test.jsx` precisa ajustar expectativas de estado inicial.
3. **Adicionar helmet** (+1 pt): `npm install helmet` e `app.use(helmet())` no `server.js` para headers de segurança.
4. **Limite de payload** (+0.5 pt): `express.json({ limit: '10kb' })` no `server.js`.
5. **Conformidade LGPD** (+2 pts): política de privacidade + endpoint de exclusão de dados do usuário.
